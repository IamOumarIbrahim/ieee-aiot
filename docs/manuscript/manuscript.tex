...
false-alarm suppression trajectories diverge by up to $31.5\times$
across architectures at intermediate ratios, yet all four converge
...
Vision systems in continuous edge AIoT sensing must process video
...
sustained negative supervision. The suppression divergence across
architectures reaches $31.5\times$ (YOLO26n vs. YOLO12n) at
$r{=}0\%$.
...
distribution focal loss receive no update from them; only the
...
\end{tabular}
\vspace{1pt}
{\footnotesize Modeled via~\eqref{eq:nuisance} with
$p_{\mathrm{neg}}{=}0.809$. Bold: lowest alert rate per frame-rate column.}
\end{table}

Without negative training, detectors trigger up to 8{,}655 nuisance
alerts per hour at 30\,FPS. Scaling negative frames to 80\% cuts
alert rates by up to 96.8\% without changing a single model
...
alert rate ($5{,}977$/hr at 30\,FPS) makes it impractical for
