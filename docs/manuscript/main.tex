...
\begin{figure*}[t]
  \centering
  \subfloat[Conv Mechanism ($R_{\mathrm{conv}} = 3\times 3$ Kernel)]{\includegraphics[width=0.22\textwidth]{fig_grid_mech_conv.pdf}\label{fig:mech_conv}}\hfil
  \subfloat[Conv Heatmap (Localized Grid FP)]{\includegraphics[width=0.22\textwidth]{fig_grid_heat_conv.pdf}\label{fig:heat_conv}}\hfil
  \subfloat[Conv Flow (Strictly Local $\nabla_W$)]{\includegraphics[width=0.22\textwidth]{fig_grid_flow_conv.pdf}\label{fig:flow_conv}}\\[-0.5ex]
  \subfloat[Attn Mechanism (Area Partitioning)]{\includegraphics[width=0.22\textwidth]{fig_grid_mech_attn.pdf}\label{fig:mech_attn}}\hfil
  \subfloat[Attn Heatmap (Global Cross-Binding)]{\includegraphics[width=0.22\textwidth]{fig_grid_heat_attn.pdf}\label{fig:heat_attn}}\hfil
  \subfloat[Attn Flow (Dense Cross-Area $\nabla_{QK}$)]{\includegraphics[width=0.22\textwidth]{fig_grid_flow_attn.pdf}\label{fig:flow_attn}}
  \caption{Architectural feature aggregation and negative gradient propagation dynamics across a $3\times 2$ comparative grid. Evaluated on an unannotated negative frame ($X \in \mathcal{D}_{\mathrm{neg}}$, $Y{=}\emptyset$), regression heads vanish and loss collapses strictly to classification BCE (\eqref{eq:det_neg_loss}). \textbf{Top row (Pure RepConv, YOLO26n):} (a) Receptive field is strictly localized to sliding-window kernels ($R_{\mathrm{conv}} = 3\times 3$); (b) Classification score heatmap shows isolated grid-cell activation on high-frequency edge texture (knuckles, $\hat{p}=0.70$); (c) Backpropagation $\nabla_W \mathcal{L}_{\mathrm{cls}}^{\mathrm{neg}}$ updates only local spatial kernels, rapidly saturating edge rejection ($r{=}40\%$) with $0.0\%$ curation gain (\textit{Sample for Convolutions}). \textbf{Bottom row (Linear Area Attention, YOLO12n):} (d) Feature maps are partitioned into horizontal/vertical visual areas; (e) Heatmap displays global area-spanning correlations binding ambient window glare to hand/face contours; (f) Backpropagation $\nabla_{QK} \mathcal{L}_{\mathrm{cls}}^{\mathrm{neg}}$ updates dense cross-area projection matrices, requiring high-entropy hard negatives to decorrelate long-range false bindings ($-65.2\%$ false-alarm drop; \textit{Curate for Attention}).}
  \label{fig:feature_aggregation_grid}
\end{figure*}
...
\end{equation}
Although this loss formulation is mathematically shared, how negative gradients propagate back through the network depends fundamentally on architectural feature aggregation (visualized across mechanism, activation heatmap, and gradient flow in Fig.~\ref{fig:feature_aggregation_grid}):
\begin{itemize}
...
In stark contrast, models equipped with attention (YOLO11n, YOLO12n) and dual-assignment heads (YOLOv10n) achieve dramatic false-alarm reductions ($55.6\%$--$71.0\%$) alongside concurrent gains in localization accuracy ($\mathrm{mAP}_{50:95}$ increasing by up to $+0.027$). Attention layers possess flexible, global hypothesis spaces. Uniform random sampling predominantly selects low-entropy background patches (plain headrests, uniform window glass) that offer negligible gradient feedback to global query-key projection weights. High-entropy hard negatives capture complex ambient reflections and driver silhouettes that provoke spurious cross-attention activations (Fig.~\ref{fig:taxonomy}), forcing attention layers to decorrelate non-target contextual patterns from true driver cues. For YOLOv10n, mined negatives penalize spurious multi-anchor activations in the auxiliary head, sharpening the decision boundary of the one-to-one inference branch.
