...
\end{tabular}
\vspace{1pt}
{\footnotesize Evaluated on 1,272 negative test frames ($\tau{=}0.25$, matched $N{=}4{,}001$ training frames). $\%\,\Delta$: relative change in $\mathrm{FP/1k}$. Bold marks best condition per architecture.}
\end{table}
...
\item \textbf{Multi-Seed Replication \& Stability:} Dual-seed replication ($\text{seed}{=}42, 43$) across all four architectures on reference splits ($0\%$--$40\%$) confirms identical suppression trajectories: YOLO11n ($67.2 \to 22.0\,\mathrm{FP/1k}$), YOLO26n ($80.6 \to 9.4\,\mathrm{FP/1k}$), YOLO12n ($45.2 \to 18.9\,\mathrm{FP/1k}$), and YOLOv10n ($57.4 \to 14.9\,\mathrm{FP/1k}$) with tight cross-seed variance ($\pm 0.56$ to $\pm 5.56\,\mathrm{FP/1k}$ across non-zero splits).
\end{itemize}
