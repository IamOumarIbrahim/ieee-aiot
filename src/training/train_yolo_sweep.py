...
def calculate_fp_per_1k(model, test_manifest_path, conf_thresh=0.25):
    ...
    val_test_dir = Path(test_manifest_path).parent.parent / "coco"
    ...
    for p in all_test_paths:
        ...
        if fn in neg_img_filenames or fn.replace("images/", "") in {f.replace("images/", "") for f in neg_img_filenames}:
            neg_test_paths.append(p)

    total_neg_frames = len(neg_test_paths)
    if total_neg_frames == 0:
        return 0.0, 0, 0

    # Run inference on negative test frames
    fp_count = 0
    results = model.predict(neg_test_paths, conf=conf_thresh, verbose=False)
    for r in results:
...
def main():
    ...
    for idx, s in enumerate(selected_splits, 1):
        ...
        # Save interim summary after each run
    ...
    print("="*80)
    print(f"Summary persisted to {project_dir / f'{model_stem}_sweep_summary.json'}\n")
